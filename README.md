...
1. `./images/`: This folder contains images that were used for testing some of the preprocessing steps. Locally, this folder also contains the training, testing, validation, and patch image files. The latter set of files wasn't pushed as they are collectively over 140 GB in size.
...
3. `preprocessing_playground.ipynb`: Contains code that was used to unzip the training images, replace damaged images with the provided patch images, remove corrupted images (turns out, there was only one such image), and tests related to rescaling/cropping images as described in the paper and mean image subtraction.
4. `preprocessing.py`: This script contains cleaned-up code from the above-mentioned Python notebook used to rescale and crop 1.2 million training images, as well as the validation and testing images.
5. `augment_ingest.ipynb`: This notebook is partly a continuation of `preprocessing_playground.ipynb` as the mean activity of all the training images was computed here and then saved. Tests related to augmentations — random sampling of image portions to expand the training dataset by a factor of 2048 and PCA on all RGB pixels — were performed in here as well. Lastly, sysnsets and lemmas related to the training labels were explored, and a CSV with image names, training labels, and other information was created and saved. *This is the bulkiest notebook in the repository.*
...
7. `alexnet_torch.ipynb`: Contains a PyTorch-ified version of the 2-GPU neural network architecture as defined in the paper, functions that augment the images on the fly, trial training runs on a subset of the entire training set, and some experimentation with the hash-based augmentation routine.
...
I worked on this project primarily from the ~25th of June to the ~8th of August, 2024. I spent a couple of hours the week of 19th of August ironing out the last doubts/issues with the code and trained the model twice — once on the 22nd of August and another on the 12th of September (with some minor fixes). The last training run, which took around two days on my university’s high-performance computer, yielded adequate results.
...
7. Zero loss issue during test runs
    ...
    - Something fundamental in the model? No
		- Tested using a slightly modified Torch version of AlexNet
	- Labels? Yes
